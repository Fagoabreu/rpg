...
public class UI {
	...
	public void renderTexts (Graphics g) {
		...
		//Ammo
		g.drawString("Municao: " + p.getAmmo() + "/" + p.getMaxAmmo(), 9*scale, (Game.getGame().HEIGHT -6) * scale);
		
		if(Game.getGame().enableDebug) {
			g.drawString("FPS:" + Game.getGame().curFPS, (Game.getGame().WIDTH/2)*scale, 11*scale);
